...
public class CardComponent : MonoBehaviour
{
	...
	private Transform standardDeck;
	private Transform discardPile;
	private Transform placedPile;
	...
	void Start()
	{
		...
		standardDeck = GameObject.Find("Deck").transform;
		discardPile = GameObject.Find("Discard Pile").transform;
		placedPile = GameObject.Find("placed pile").transform;
	...
	void FixedUpdate()
	{
		if (Unlocked.Contains(true))
		{
			...
			else if (Dragging)
			{
				...
				float Distance1 = Mathf.Sqrt(Mathf.Pow(transform.position.x- standardDeck.position.x, 2) + Mathf.Pow(transform.position.y + standardDeck.position.y, 2));
				float Distance2 = Mathf.Sqrt(Mathf.Pow(transform.position.x-discardPile.position.x, 2) + Mathf.Pow(transform.position.y + -discardPile.position.y, 2));
				float Distance3 = Mathf.Sqrt(Mathf.Pow(GameManagerObject.GetComponent<GameManagerComponent>().GetPlayedCardsCount() * 50 - transform.position.x, 2) + Mathf.Pow(transform.position.y - placedPile.position.y
				, 2));
